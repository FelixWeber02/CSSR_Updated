...
struct JointData {
    typedef std::array<FloatType, NPAST*base_sample_dim> DataType;
    typedef std::array<FloatType, NFUTURE*base_sample_dim>  PredictionType;
...
template<int dim> struct Utility {
    FloatType operator()(const PredictionType& p1, const PredictionType& p2) {
        FloatType negssq = 0;
        for (int i=0; i < std::min(p1.size(),p2.size()); ++i) negssq -= (p1[i] - p2[i])*(p1[i] - p2[i]);
        return negssq;
    }
};
...
int main(int argc, char**argv) {
    ...
    if (argc<5) {
        cout << "Arguments expected:  n_past_samples n_future_samples kernel_width_or_0  flag  output_dir  input_file1  [input_file2 [...]]" << endl;
    ...
    //int N_past_samples = atoi(argv[1]);
    //int N_future_samples = atoi(argv[2]);
    ...
    for (int i=0; i<nullPrediction.size(); ++i) nullPrediction[i] = 0;
    ...
    if ((flag&2)!=0) {
        ...
        for (argi = argifiles; argi<argc; ++argi) {
            ...
            double number;
            std::array<double, base_sample_dim> prev;
    ...
    for (argi = argifiles; argi<argc; ++argi) {
        ...
        SimpleGaussianKernel<FloatType> pkernel(ksizej,sizeof(PredictionType),1e-9f);
        ...
        SimpleGaussianKernel<FloatType> dkernel(ksizej,sizeof(DataType),1e-9f);
        ...
        analyser.applyUtility( Utility<sizeof(PredictionType)>() );
        cout << " done!" << endl;
